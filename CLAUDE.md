...
1. [docs/library-first-redesign.md](docs/library-first-redesign.md) — 設計とマイルストーン、進捗。
2. 関連する `server/` / `frontend/` の既存実装。
3. 今回の依頼が設計のどこに関係するかを把握してから作業する。方針と矛盾しそうな場合は実装前に確認する。
...
- 設計・進捗は [docs/library-first-redesign.md](docs/library-first-redesign.md) に集約する。
